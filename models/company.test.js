...
describe("findAll with filters", function () {
  ...
  test("works with minEmployees filter", async function () {
    ...
    expect(companies).toEqual(expect.arrayContaining([
      expect.objectContaining({ numEmployees: expect.any(Number) })
    ]));
  ...
  test("works with maxEmployees filter", async function () {
    ...
    companies.forEach(company => {
      expect(company.numEmployees).toBeLessThanOrEqual(2);
    });
  });

  test("works with name and minEmployees filters", async function () {
    const companies = await Company.findAll({ name: "C", minEmployees: 1 });
    expect(companies).toEqual(expect.arrayContaining([
      expect.objectContaining({
        name: expect.stringContaining("C"),
  ...
  test("throws BadRequestError if minEmployees is greater than maxEmployees", async function () {
    await expect(Company.findAll({ minEmployees: 4, maxEmployees: 2 })).rejects.toThrow(BadRequestError);
  });
});
